<script lang="ts">
  ...
  async function fetchProducts() {
    ...
    try {
      loading = true
      error = null
      const result = await productService.getProducts(page, query, pageSize, abortController.signal)
...
<div class="space-y-6">
  ...
  {:else if error}
    <div class="text-red-700 bg-red-50 border border-red-200 rounded-lg p-4">{error}</div>
  {:else}
    <!-- ตารางแบบเรียบง่าย -->
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      <div class="px-4 py-3 bg-gray-50 border-b border-gray-300 text-sm text-gray-600">
        ...
        <table class="min-w-full text-sm">
          ...
          <tbody>
            {#if products.length === 0}
              <tr>
                <td colspan="6" class="px-4 py-10 text-center text-gray-500">ไม่พบสินค้า</td>
              </tr>
            {:else}
              {#each products as p}
                <tr class="border-t border-gray-100 hover:bg-gray-50">
                  <td class="px-4 py-3">
                    <div class="h-14 w-14 bg-gray-200 rounded-lg flex items-center justify-center relative overflow-hidden">
                      ...
                      {:else}
                        ...
                        <svg class="w-7 h-7 text-gray-400" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                  ...
                  <td class="px-4 py-3 text-gray-700">{p.category_name || 'ไม่ระบุ'}</td>
                  <td class="px-4 py-3 text-gray-900">{formatPrice(p.price)}</td>
                  <td class="px-4 py-3 text-gray-600">{formatDate(p.created_at)}</td>
                  <td class="px-4 py-3 text-gray-600">{formatDate(p.updated_at)}</td>
                  <td class="px-4 py-3 text-right flex gap-2 justify-end">
                    <button type="button" class="px-3 py-1.5 rounded-md bg-white border border-sky-300 text-sky-700 hover:bg-sky-50 cursor-pointer" onclick={() => openView(p.id)}>ดู</button>
                    <a href={`/admin/products/update?id=${p.id}`} class="px-3 py-1.5 rounded-md bg-white border border-blue-300 text-blue-700 hover:bg-blue-50 cursor-pointer">แก้ไข</a>
                    <button type="button" class="px-3 py-1.5 rounded-md bg-white border border-red-300 text-red-700 hover:bg-red-50 cursor-pointer" onclick={() => askDelete(p)}>ลบ</button>
                  </td>
                </tr>
              {/each}
            {/if}
      ...
      <!-- แบ่งหน้าแบบเลขหน้า + ... -->
      <div class="flex items-center justify-between px-4 py-3 bg-gray-50 border-t border-gray-300">
        <button
          type="button"
        ...
        <nav class="flex items-center gap-1" aria-label="Pagination">
          {#each pageItems as item}
            {#if item === '...'}
              <span class="px-3 h-9 inline-flex items-center justify-center text-gray-400 select-none">...</span>
            {:else}
              <button
                type="button"
...
{#if showModal}
  <div class="fixed inset-0 z-50 flex items-center justify-center">
    ...
    <div class="relative bg-white rounded-xl shadow-xl w-[min(92vw,720px)] max-h-[90vh] overflow-auto">
      <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
        <h2 class="text-lg font-semibold">รายละเอียดสินค้า</h2>
        <button type="button" class="p-2 rounded-md hover:bg-gray-100" aria-label="ปิด" onclick={closeModal}>
      ...
      <div class="p-4 border-t border-gray-200">
        {#if modalLoading}
          <div class="flex items-center gap-3 text-gray-600"><div class="animate-spin rounded-full h-5 w-5 border-2 border-blue-600 border-t-transparent"></div>กำลังโหลด...</div>
        {:else if !selected}
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="md:col-span-1">
              <div class="aspect-square bg-gray-100 rounded-lg overflow-hidden flex items-center justify-center">
                {#if selected.image}
                  <img src={selected.image} alt={selected.name} class="h-full w-full object-cover" onerror={(e) => (e.currentTarget as HTMLImageElement).style.display = 'none'} />
                {:else}
                  <svg class="w-10 h-10 text-gray-400" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7l9-4 9 4-9 4-9-4z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 7v10l-9 4-9-4V7" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 17l9-4 9 4" /></svg>
                {/if}
              </div>
            </div>
            <div class="md:col-span-2 space-y-2">
              <div class="text-xl font-semibold text-gray-900">{selected.name}</div>
              <div class="text-gray-700">{selected.description}</div>
              <div class="text-gray-900 font-medium">ราคา: {formatPrice(selected.price)}</div>
              <div class="text-gray-600">หมวดหมู่: {selected.category_name || selected.category_id}</div>
              <div class="text-gray-600">วันที่สร้าง: {formatDate(selected.created_at)}</div>
            </div>
          </div>
        {/if}
      </div>
...
{#if confirmOpen}
  <div class="fixed inset-0 z-50 flex items-center justify-center">
    <div class="absolute inset-0 bg-black/40" onclick={closeConfirm} aria-hidden="true"></div>
    <div class="relative bg-white rounded-xl shadow-xl w-[min(92vw,480px)]">
      ...
      <div class="px-6 py-4 space-y-2">
        <p class="text-gray-700">คุณต้องการลบสินค้า <span class="font-medium">{deleteName}</span> ใช่ไหม?</p>
        {#if deleteError}
          <div class="text-red-700 bg-red-50 border border-red-200 rounded-lg p-3">{deleteError}</div>
        {/if}
      </div>
      <div class="px-6 py-4 border-t border-gray-200 flex justify-end gap-2">
        <button type="button" class="px-4 h-10 rounded-lg bg-gray-100 text-gray-700 hover:bg-gray-200" onclick={closeConfirm} disabled={deleting}>ยกเลิก</button>
        <button type="button" class="px-4 h-10 rounded-lg bg-red-600 text-white hover:bg-red-700 disabled:opacity-50" onclick={confirmDelete} disabled={deleting}>{deleting ? 'กำลังลบ...' : 'ลบ'}</button>
      </div>
    </div>
  </div>
{/if}
